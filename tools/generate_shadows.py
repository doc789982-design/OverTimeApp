...
SOFT_LEVELS = {
    # (размытие, радиус угла). Вылет (pad) = размытие*2+2:
    #   L1 → 10px (поля списка сотрудников 12px — тень не обрезается)
...
INSET_LEVELS = {
    # Радиус угла = радиус элемента в приложении:
    #   6  = AppTheme.radiusSmall (квадратик чекбокса)
    #   12 = пилюля (дорожка переключателя)
    1: (3, 6),     # чекбоксы и небольшие «ямки»
    2: (3, 12),    # дорожка переключателя (пилюля)
}
